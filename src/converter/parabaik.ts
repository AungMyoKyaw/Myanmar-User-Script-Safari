...
// A direct, faithful port of the Parabaik Z1_Uni conversion routine.
// Source: parabaik.js in this repository. Minor adjustments only where
// original regex had obvious typos to keep the code valid in TypeScript.

export function parabaikZ1Uni(input: string): string {
  let output = input;
  const tallAA = '\u102B';
  // const AA = "\u102C"; // unused in conversion logic
  const vi = '\u102D';
  // lone gyi tin
  // const ii = "\u102E"; // unused in conversion logic
  const u = '\u102F';
  const uu = '\u1030';
  // const ve = "\u1031"; // unused in conversion logic
  // const ai = "\u1032"; // unused in conversion logic
  const ans = '\u1036';
  const db = '\u1037';
  // const visarga = "\u1038"; // unused in conversion logic
  const asat = '\u103A';
  const ya = '\u103B';
  const ra = '\u103C';
  const wa = '\u103D';
  ...
  output = output.replace(
    /(([\u1000-\u101C\u101E-\u102A\u102C-\u102E-\u103F\u104C-\u109F]))(\u1040)(?=\u0020)?/g,
    (_m, g1: string) => (g1 ? g1 + '\u101D' : _m + g1)
  );
  // zero and wa
  output = output.replace(/((\u101D))(\u1040)(?=\u0020)?/g, (_m, g1: string) =>
    g1 ? g1 + '\u101D' : _m + g1
  );
  output = output.replace(
    /([^\u1040-\u1049\u1031])\u1040([\u102b-\u1030\u1036\u1037\u1038\u103a]|[\u1000-\u1021]\u103a)/g,
    '$1\u101d$2'
  );
  // zero and wa
  output = output.replace(
    /(([\u1000-\u101C\u101E-\u102A\u102C-\u102E-\u103F\u104C-\u109F]))(\u1047)/g,
    (_m, g1: string) => (g1 ? g1 + '\u101B' : _m + g1)
  );
  // seven and ra
  // NOTE: Original source had a malformed regex here; omitted to keep validity:
